...
export interface DepthCarouselItem {
  image?: string;
  alt?: string;
  /** Rich slide content (used instead of <img> when provided). */
  content?: ReactNode;
}
...
const normalizeItem = (
  it: string | DepthCarouselItem,
): DepthCarouselItem => (typeof it === 'string' ? { image: it, alt: '' } : it);
...
const DepthCarousel = ({
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const measure = () => {
      // clientWidth is synchronous (unlike observer delivery), so the
      // correct scale applies on the very first paint — incl. phones.
      const w = root.clientWidth;
      const cfg = cfgRef.current;
      // Phones: only budget room for one side of neighbours + tighter
      // padding so the focused card stays large.
      const narrow = w < 640;
      const sideRoom = Math.abs(cfg.spread) * (narrow ? 1 : 2);
      const padding = narrow ? 48 : 120;
      const needed = cfg.cardWidth + sideRoom + padding;
      scaleRef.current = clamp(w / needed, narrow ? 0.55 : 0.4, 1);
      layout(posRef.current);
    };
    measure();
  ...
  // NOTE: no wheel handler — mouse-wheel must scroll the page, never the
  // carousel. Navigation is buttons / dots / swipe / keyboard only.
